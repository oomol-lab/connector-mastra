...
| File | Covers |
| --- | --- |
| [`agent.ts`](./agent.ts) | Code-configured agent: `listTools` browsing, `resolveTools` on the default connection (or `OOMOL_CONNECTION_NAME`), the resulting `tools` map on a Mastra `Agent` |
